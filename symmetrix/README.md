...
The output JSON is the file used by the ASE calculator. MACEField JSON currently
runs through the native serial Symmetrix evaluator, so use `dtype="float64"`.
If `use_kokkos=True` is requested for a field-coupled JSON model, the calculator
falls back to the serial field path because Kokkos field-response kernels are not
implemented yet.
