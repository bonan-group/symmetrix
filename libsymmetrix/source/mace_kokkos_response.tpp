    ...
    Kokkos::View<const double*> electric_field)
{
    ...
    if (response_mode == MACEStreamedEdgesMode::all)
        compute_R0(num_nodes, node_types, num_neigh, neigh_types, r);
    if (response_mode != MACEStreamedEdgesMode::legacy)
    ...
    for (int seed=0; seed<3; ++seed) {
        ...
        const int response_edges = neigh_indices.extent(0);
        ...
        if (response_mode != MACEStreamedEdgesMode::legacy
            ...
            Kokkos::parallel_for(
                ...
                Kokkos::TeamPolicy<>(
                    ...
                    edges_per_team, 32),
                KOKKOS_LAMBDA (Kokkos::TeamPolicy<>::member_type team) {
                    const int edge_begin = team.league_rank()*edges_per_team;
                    const int edge_count = Kokkos::min(
                        edges_per_team, response_edges-edge_begin);
                    Kokkos::parallel_for(
                        Kokkos::TeamThreadRange(team, edge_count),
                                    ...
                                    for (int path=0; path<phi_paths; ++path) {
                                        ...
                                        for (int row=row_begin; row<row_end; ++row) {
                                            ...
                                            local_x -= adjoint_dot*(
                                                ...
                                                +angular_base*harmonics_grad(
                                                    3*edge*lm_count+lm1));
                                            local_x -= base_adjoint*(
                                                radial_dot*x_over_r
                                                +angular_dot*harmonics_grad(
                                                    3*edge*lm_count+lm1));
                                            local_y -= adjoint_dot*(
                                                radial_base*y_over_r
                                                +angular_base*harmonics_grad(
                                                    (3*edge+1)*lm_count+lm1));
...
#ifdef KOKKOS_ENABLE_CUDA
        if (response_mode != MACEStreamedEdgesMode::legacy
            && streamed_edge_receivers.extent(0) == response_edges) {
            constexpr int edges_per_team = 8;
            const auto edge_receivers = streamed_edge_receivers;
            Kokkos::parallel_for(
                                    ...
                                    for (int l=0; l<=lmax; ++l) {
                                        ...
                                        for (int lm=l*l; lm<(l+1)*(l+1); ++lm) {
                                            ...
                                            local_x -= radial_force*x_over_r
                                                +angular_force*harmonics_grad(
                                                    3*edge*lm_count+lm);
                                            local_y -= radial_force*y_over_r
                                                +angular_force*harmonics_grad(
                                                    (3*edge+1)*lm_count+lm);
                                            local_z -= radial_force*z_over_r
                                                +angular_force*harmonics_grad(
                                                    (3*edge+2)*lm_count+lm);
                                        }
                                    }
                                }, force_x, force_y, force_z);
                            Kokkos::single(Kokkos::PerThread(team), [=]() {
    ...
    if (response_mode == MACEStreamedEdgesMode::r1
        || response_mode == MACEStreamedEdgesMode::all) {
        R1 = decltype(R1)();
